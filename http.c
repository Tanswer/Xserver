...
/* 静态请求回复 */
static void serve_static(int fd, char *filename, size_t filesize, mm_http_out_t *out);
...
{
    mm_http_request_t *r = (mm_http_request_t *)ptr;
    ...
    while(1){ /* 循环读取客户数据并分析之 */
        ...
        if(n == 0){
            ...
            rc = mm_http_close_conn(r);
            check(rc == 0, "do_request:mm_http_close_conn");
        }
        if(n < 0){
            if( errno != EAGAIN ){      /* 出错 */
        ...
        rc = mm_http_parse_request_line(r);
        if(rc == MM_AGAIN){
            continue;
        }else if(rc != MM_OK){
            log_err("rc != MM_OK");
        ...
        /* 处理HTTP Header */
        mm_http_out_t *out = (mm_http_out_t *)malloc(sizeof(mm_http_out_t));
        if(out == NULL){
            log_err("no enough space for mm_http_out_t");
            exit(1);
        }

        rc = mm_init_out(out, fd);
        check(rc == MM_OK,"mm_init_out");
    ...
    mm_epoll_mod(r->m_epfd, r->m_sockfd, &event);
    //mm_add_timer(r, T)
    return ;
}
...
static const char *get_filetype(const char* type)
{
    ...
    for(int i=0; mm_type[i].type != NULL; i++){
        if(strcmp(type,mm_type[i].type) == 0)
            return mm_type[i].value;
    }
}
